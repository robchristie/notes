...
def iou_xyxy(a, b):
    ...
    iw, ih = max(0, xa1-xa0), max(0, ya1-ya0)
    inter = iw*ih
...
def eval_prompts(coco_gt_json, det_json, prompt_to_cat, iou_thresh=0.5, ks=(1,5,10)):
    ...
    preds = json.load(open(det_json,'r'))
    img2gt = {}
    ...
    for prompt, cat_id in prompt_to_cat.items():
        ...
        if not dets:
            results[prompt] = {'AP': 0.0, **{f'P@{k}':0.0 for k in ks}}
            continue
        ...
        for iid, bb, sc in dets:
            ...
            ious = [iou_xyxy(bb, g) for g in gtlist]
        ...
        for k in ks:
            topk = dets[:k]
            hits = 0
            for iid,bb,sc in topk:
                gtlist = img2gt.get((iid, cat_id), [])
                if any(iou_xyxy(bb,g)>=iou_thresh for g in gtlist):
                    hits += 1
...
if __name__ == '__main__':
    ...
    ap.add_argument('coco_gt'); ap.add_argument('det_json'); ap.add_argument('prompt_to_cat_json')
    ap.add_argument('--iou_thresh', type=float, default=0.5); ap.add_argument('--out_json', default=None)
    args = ap.parse_args()
    ptc = json.load(open(args.prompt_to_cat_json,'r'))
    out = eval_prompts(args.coco_gt, args.det_json, ptc, iou_thresh=args.iou_thresh)
