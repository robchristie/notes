...
class OVMatcher:
    ...
    def embed_text(self, prompts):
        ...
        _, txt_f, _ = self.model(None, toks)
    ...
    def score(self, crops, text_prompts=None, visual_prototypes=None):
        feats = self.embed_images(crops)
        scores = {}
        if text_prompts:
            txt = self.embed_text(text_prompts)
            scores['text'] = feats @ txt.t()
        if visual_prototypes:
            vp = self.embed_images(visual_prototypes)
            scores['visual'] = feats @ vp.t()
        return scores
